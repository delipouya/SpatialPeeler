...
### run wilcoson test between cluster-1 (true case samples) and cluster-0
#identify the uniion of the mask: cluster_mask_dict, in a way that if a cell is assigned to cluster-1 in any of the factors, it will be included in the final mask
## with out using reduce:
final_mask = np.zeros(adata.n_obs, dtype=bool)
removed_item = cluster_mask_dict.pop("phat_cluster_factor19")

for mask in cluster_mask_dict.values():
    final_mask |= (mask == '1').values

## convert true/false to 1/0
final_mask = final_mask.astype(int)

print(f"Current number of cells in final mask: {final_mask.sum()}")
print(f"Number of cells in final mask: {final_mask.sum()}")

# convert final_mask to a pd.Series to be able to use it in adata.obs
final_mask_series = pd.Series(final_mask, index=adata.obs_names)
adata.obs['final_mask'] = final_mask_series.astype(str)
...
plot.plot_grid_upgrade(adata_by_sample, sample_ids, key='final_mask',
                       title_prefix="Final Clusters", from_obsm=False, discrete=True,
